...
export default async function handler(req: Request): Promise<Response> {
  ...
  try {
    ...
    if (!txRes.ok) {
      ...
      return new Response(JSON.stringify({ error: t || `Fetch transaction failed: ${txRes.status}` }), { status: 400 })
    }
    ...
    if (!profRes.ok) {
      ...
      return new Response(JSON.stringify({ error: t || `Fetch profile failed: ${profRes.status}` }), { status: 400 })
    }
    ...
    if (!recipientCode) {
      return new Response(JSON.stringify({ error: 'Seller recipient not set' }), { status: 400 })
    }
    const platformFee = Math.round(amountNgn * 0.03 * 100)
    const amountKobo = Math.round(amountNgn * 100) - platformFee
    if (amountKobo <= 0) {
      return new Response(JSON.stringify({ error: 'Invalid payout amount' }), { status: 400 })
    }
    ...
    if (!transferRes.ok) {
      return new Response(JSON.stringify({ error: transferJson?.message || 'Payout failed' }), { status: 400 })
    }
    ...
    if (!updateRes.ok) {
      ...
      return new Response(JSON.stringify({ error: t || `Update failed: ${updateRes.status}` }), { status: 400 })
    }
    const updated = await updateRes.json()
    return new Response(JSON.stringify({ ok: true, transfer: transferJson, transaction: updated }), { status: 200 })
  } catch (e: any) {
